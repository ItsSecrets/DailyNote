...
module.exports = {
  ...
  toJson: function (src, dest, config) {
    ...
    if (!ignoreList.includes(excelName)) {
      parseWorkbook(workbook, dest, headIndex, uglifyJson, excelName, config);
    }else{
      console.log("ignore file name: ", excelName);
    }
...
function parseWorkbook(workbook, dest, headIndex, uglifyJson, excelName, config) {
  ...
  workbook.forEach(sheet => {
    ...
    if (sheet.name.indexOf('@') === -1 && sheet.name[0] !== "#" && !isOver) {
      ...
      console.log("======>>>>>>>targetNme: ",targetNme);
      ...
      // resultJson = resultJson.replace(/\n/g, "<br>");  
      fs.writeFile(dest_file, resultJson, {
            flag: "w+"
          }, err => {
        if (err) {
  ...
  config.dts += dtsstring;
  console.log("d.ts file", dest, excelName, config.dts);
  ...
  if (config.dts != undefined && config.dts != "") {
    let dest_file_dts = path.resolve(dest, "config.d.ts");
    fs.writeFile(dest_file_dts, config.dts, err => {
      if (err) {
        console.error("error：", err);
        throw err;
      }
...
function parseRow(row, rowIndex, head) {
  ...
  row.forEach((cell, index) => {
    ...
    console.log("======>>>>>>type: ", JSON.stringify(head.types));
    ...
    switch (type) {
      ...
      case DataType.DATE:
        ...
        break;
      case DataType.STRING:
        result[name] = cell.toString().replace(/\r\n/g, "\n")
        break;
      case DataType.DOUBLE:
      case DataType.FLOAT:
      case DataType.INT:
